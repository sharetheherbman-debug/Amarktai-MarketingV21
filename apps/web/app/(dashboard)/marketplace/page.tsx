...
interface MarketplaceItem { id: string; name: string; description: string | null; category: string; publisher_name: string; version: string; is_free: boolean; price_cents: number; currency?: string; install_count: number; rating_average: number; rating_count: number; tags: string[]; }
...
interface Checkout { purchase_id: string; checkout_session_id: string; checkout_url: string; }
interface PurchaseStatus { status: string; item_name: string; installation_id: string | null; error_message: string | null; }
...
export default function MarketplacePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const verify = async () => {
      setNotice('Payment received. Confirming purchase and installing the package…');
      for (let attempt = 0; attempt < 12 && !cancelled; attempt++) {
        try {
          const response = await api.get<ApiResponse<PurchaseStatus>>('/marketplace/purchases/status', { params: { session_id: sessionId } });
          if (response.data.installation_id) {
            setNotice(`${response.data.item_name} was purchased and installed successfully.`);
            await load();
            window.history.replaceState({}, '', '/marketplace');
            return;
          }
          if (response.data.status === 'failed' || response.data.error_message) {
            setError(response.data.error_message || 'Payment completed but installation failed.');
            window.history.replaceState({}, '', '/marketplace');
            return;
          }
        } catch (caught) {
          if (attempt === 11) setError(caught instanceof Error ? caught.message : 'Could not confirm marketplace purchase.');
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
      if (!cancelled) setNotice('Payment is still being confirmed. Refresh this page shortly; the signed Stripe webhook will finish installation.');
    };
    void verify();
  ...
  const installOrPurchase = async (item: MarketplaceItem) => {
    setBusyId(item.id);
    setError(null);
    try {
      if (!item.is_free && item.price_cents > 0) {
        const response = await api.post<ApiResponse<Checkout>>(`/marketplace/items/${item.id}/checkout`, { body: {} });
        window.location.assign(response.data.checkout_url);
        return;
      }
      await api.post(`/marketplace/items/${item.id}/install`, { body: { config: {} } });
      setNotice(`${item.name} installed successfully.`);
      await load();
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Marketplace action failed.'); }
  ...
  return <div className="space-y-6">
    <header><h1 className="text-2xl font-bold text-white">Marketplace</h1><p className="mt-1 text-sm text-zinc-400">Install free packages directly or purchase paid packages through signed Stripe Checkout.</p></header>
    {notice && <div className="flex items-center gap-3 rounded-lg border border-emerald-500/20 bg-emerald-500/5 px-4 py-3"><CheckCircle2 className="h-4 w-4 text-emerald-400" /><p className="text-sm text-emerald-300">{notice}</p><button type="button" onClick={() => setNotice(null)} className="ml-auto text-emerald-400"><X className="h-4 w-4" /></button></div>}
